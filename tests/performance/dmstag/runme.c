...
typedef enum {STAGTEST,DATEST1,DATEST2} Test;
...
int main(int argc,char **argv)
{
  ...
  if (test == STAGTEST) {
    ierr = DMStagCreate3d(PETSC_COMM_WORLD,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,Nx,Ny,Nz,PETSC_DECIDE,PETSC_DECIDE,PETSC_DECIDE,dof0,dof1,dof2,dof3,DMSTAG_GHOST_STENCIL_STAR,stencilWidth,NULL,NULL,NULL,&dm);CHKERRQ(ierr);
  ...
  ierr = DMSetUp(dm);CHKERRQ(ierr);
    ...
    ierr = DMDACreate3d(PETSC_COMM_WORLD,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DMDA_STENCIL_STAR,Nx,Ny,Nz,PETSC_DECIDE,PETSC_DECIDE,PETSC_DECIDE,dof0+3*dof1+3*dof2+dof3,stencilWidth,NULL,NULL,NULL,&dm);CHKERRQ(ierr);
  ierr = DMSetFromOptions(dm);CHKERRQ(ierr); // Use sparingly (for debugging)! Tests should compare the same functionality, remember.
  ierr = DMSetUp(dm);CHKERRQ(ierr);
  } else if (test == DATEST2) {
    i = 0;
    ierr = DMDACreate3d(PETSC_COMM_WORLD,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DMDA_STENCIL_STAR,Nx,Ny,Nz,PETSC_DECIDE,PETSC_DECIDE,PETSC_DECIDE,dof0,stencilWidth,NULL,NULL,NULL,&dms[i]);CHKERRQ(ierr);
    ...
    for (i=1; i<4; ++i) {
      ierr = DMDACreate3d(PETSC_COMM_WORLD,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DMDA_STENCIL_STAR,Nx,Ny,Nz,PETSC_DECIDE,PETSC_DECIDE,PETSC_DECIDE,dof1,stencilWidth,NULL,NULL,NULL,&dms[i]);CHKERRQ(ierr);
      ierr = DMSetFromOptions(dms[i]);CHKERRQ(ierr); // Use sparingly (for debugging)! Tests should compare the same functionality, remember.
      ierr = DMSetUp(dms[i]);CHKERRQ(ierr);
    }
    for (; i<7; ++i) {
      ierr = DMDACreate3d(PETSC_COMM_WORLD,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DMDA_STENCIL_STAR,Nx,Ny,Nz,PETSC_DECIDE,PETSC_DECIDE,PETSC_DECIDE,dof2,stencilWidth,NULL,NULL,NULL,&dms[i]);CHKERRQ(ierr);
      ierr = DMSetFromOptions(dms[i]);CHKERRQ(ierr); // Use sparingly (for debugging)! Tests should compare the same functionality, remember.
      ierr = DMSetUp(dms[i]);CHKERRQ(ierr);
    }
    ierr = DMDACreate3d(PETSC_COMM_WORLD,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DMDA_STENCIL_STAR,Nx,Ny,Nz,PETSC_DECIDE,PETSC_DECIDE,PETSC_DECIDE,dof0,stencilWidth,NULL,NULL,NULL,&dms[i]);CHKERRQ(ierr);
